...
<?php
  ...
  {
    $messages = array("Ürünün adı boş bırakılamaz", "Ürünün adının uzunluğu 100 karakterden büyük olamaz", "Ürünün adı Türkçe karakter içeremez", "Ürünün kategorisi boş bırakılamaz",
    "Ürünün kategorisinin uzunluğu 100 karakterden büyük olamaz", "Ürünün kategorisi rakam içeremez", "Uzantı desteklenmiyor, Lütfen JPEG, JPG veya PNG uzantılı dosya seçiniz.",
    "Dosya boyutu 16MB'dan küçük olmalıdır.", "Ürün bilgisi boş bırakılamaz", "Ürün bilgisinin uzunluğu 512 karakterden büyük olamaz", "Hatalı ürün bilgisi", 
    "Ürünün fiyatı boş bırakılamaz", "Ürünün fiyatı 10 basamaktan büyük olamaz", "Ürün fiyatı hatalı girilmiştir", "Ürün eklenemedi"); 
    $status = array("danger","success");
    
    // Alert message
    $addproduct_alert_message = array(
      "message" => $messages[$error_index],
      "type" => $status[$status_index]
    );
    $_SESSION["addproduct_alert_message"] = $addproduct_alert_message;
    header("location: addproduct.php?productadd=TRUE");
  }

  // Click control
  if($_GET["productadd"] == TRUE)
  {
    ...
    if(isset($_POST['productaddbtn'])) 
    {
      ...
      if($error_counter == 0)
      {
        $query = $conn->prepare("INSERT INTO product SET
        product_name = ?,
        product_category = ?,
        product_image = ?,
        product_information = ?,
        product_price = ?");
        $insert = $query->execute(array($product_name, $product_category, $image, $product_information, $product_price));
        ...
        <div id="content">
          ...
          <div class="container-fluid">
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
              ...
              <!-- Error Message -->
              <?php if(isset($_SESSION["addproduct_alert_message"])) { ?>
                <div class="alert alert-<?php echo $_SESSION["addproduct_alert_message"]["type"]?> mt-4">
                  <?php echo $_SESSION["addproduct_alert_message"]["message"] ?>
                </div>
                <?php unset($_SESSION["addproduct_alert_message"]); ?>
              <?php } ?>
              <!-- Error Message -->
            </div>
            ...
            <div class="card shadow mb-4">
              ...
              <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Ürün Kaydı Ekle</h6>
              </div>
              <!-- Header -->
              <!-- Body -->
              <div class="card-body">
              <form method="POST" autocomplete="off" enctype="multipart/form-data">
                  ...
                  <div class="card-body">
                    ...
                    <div class="form-group user-box mt-4 mb-3">
                      <input type="text" maxlength="100" name="product_name" placeholder="Ürün Ad" required/>
                      <span for="productid-label">Ürün Ad</span>
                    </div>
                    <!-- Product Name -->   
                    <!-- Product Category -->
                    <div class="form-group user-box mb-3">
                      <input type="text" maxlength="100" list="prodlist" name="product_category" placeholder="Ürün Kategori" required/>
                      <datalist id="prodlist">
                          <?php
                            $pquery = $conn->query("SELECT DISTINCT product_category FROM product", PDO::FETCH_ASSOC); 
                            if ($pquery->rowCount())
                            {
                              foreach($pquery as $prow) 
                              {
                                echo "<option value='".$prow['product_category']."'></option>";
                              }
                            }                 
                          ?>
                        </datalist>
                    ...
                    <!-- Product Image -->
                    <div class="form-group user-box mb-3">
                      <input type="text" maxlength="512" name="product_information" placeholder="Ürün Bilgisi" required/>
                      <span for="paymenttotal-label">Ürün Bilgisi</span>
                    </div>                      
                    <!-- Product Image -->
                    <!-- Price -->
                    <div class="form-group user-box mb-3">
                      <input type="number" maxlength="10" name="product_price" placeholder="Ücret" required/>
                  ...
                  <!-- Button -->  
                  <button type="submit" class="btn btn-block mt-3 mb-3 text-light"  style="border-color:#FF3100; background-color: #FF3100;" value="Submit" name="productaddbtn">EKLE</button> 
                  <!-- Button -->     
                </form>
